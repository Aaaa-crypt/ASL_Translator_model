...
MODEL_PATH = "asl_seq_model_smart_v8.keras"
HISTORY_PATH = "history_seq_smart_v8.json"
...
def load_tf_dataset(split="train", class_names=None, max_classes=None):
    ...
    # Shuffle, batch, prefetch, and repeat indefinitely
    dataset = dataset.shuffle(500).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE).repeat()
    steps_per_epoch = max(1, len(dataset_list) // BATCH_SIZE)
    return dataset, class_names, steps_per_epoch

# ---------------- LOAD DATA ----------------
print("[INFO] Loading training data...")
train_ds, class_names, train_steps = load_tf_dataset("train", max_classes=MAX_CLASSES)
print("[INFO] Loading validation data...")
val_ds, _, val_steps = load_tf_dataset("val", class_names=class_names)
...
plt.tight_layout()
plt.savefig("training_curves_seq_smart_v8.png")
plt.show()
