# ------------------------ training_model3_with_test_optimized.py ------------------------
import os
import json
...
MAX_CLASSES = 100
MODEL_PATH = "asl_seq_model_smart_v3_optimized.keras"
HISTORY_PATH = "history_seq_smart_v3_optimized.json"
TEST_JSON_PATH = "test_metrics_v3_optimized.json"
TEST_PLOT_PATH = "test_graphs_v3_optimized.png"
...
def load_clip(frame_paths, label, augment=True):
    clip = []
...
def tf_load_clip(frame_paths, label, augment=True):
    clip, lbl = tf.py_function(load_clip, [frame_paths, label, augment], [tf.float32, tf.float32])
    clip.set_shape([FRAMES_PER_CLIP, IMG_SIZE[0], IMG_SIZE[1], 3])
    lbl.set_shape([num_classes])
    return clip, lbl

def create_tf_dataset(split="train", augment=True):
    ...
    dataset = dataset.map(lambda x, y: tf_load_clip(x, y, augment), num_parallel_calls=tf.data.AUTOTUNE)
    if split=="train":
        dataset = dataset.shuffle(500)
    dataset = dataset.batch(BATCH_SIZE).cache().prefetch(tf.data.AUTOTUNE).repeat()
    
    # Steps per epoch ensures full coverage of dataset
    steps = max(1, len(clip_paths) // BATCH_SIZE)
    return dataset, class_names, steps

# ---------------- LOAD DATA ----------------
print("[INFO] Loading training data...")
train_ds, class_names, train_steps = create_tf_dataset("train", augment=True)
print("[INFO] Loading validation data...")
val_ds, _, val_steps = create_tf_dataset("val", augment=False)
print("[INFO] Loading test data...")
test_ds, _, test_steps = create_tf_dataset("test", augment=False)
...
plt.tight_layout()
plt.savefig("training_graphs_v3_optimized.png")
plt.show()
print("[INFO] Training graphs saved to training_graphs_v3_optimized.png")

# ---------------- TEST EVALUATION ----------------
print("[INFO] Evaluating on test set...")
test_metrics = model.evaluate(test_ds, steps=test_steps, verbose=1)
test_results = {"loss": test_metrics[0], "accuracy": test_metrics[1]}
